...
import io, aiohttp.client_exceptions, aiohttp, re, json, time, asyncio, random
from fake_useragent import UserAgent

TOKEN = '7191525489:AAH6QY--mHZ6ZSKdGPporZa7R_jKKP8KTPk'
...
async def sh(message):
    ...
    async with aiohttp.ClientSession() as r:
        ...
        async with r.get('https://dogoodshop.org/cart.js', headers=headers) as response:
        ...
        async with r.post('https://dogoodshop.org/checkouts/unstable/graphql', params=params, headers=headers, json=json_data) as response:
            raw = await response.text()
            try:
                res_json = json.loads(raw)
...
"""
            ...
            for _ in range(max_retries):
                async with r.post('https://dogoodshop.org/checkouts/unstable/graphql',params=params,headers=headers,json=json_data) as final_response:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Halo! Kirim file .txt berisi data kartu, saya akan memprosesnya.")
